...
(async()=>{
 ...
 await cohort.click();await page.getByRole('option',{name:'2025级',exact:true}).click();
 await poll(async()=>(await data('students')).length===0);
 await cohort.click();await page.getByRole('option',{name:'2026级',exact:true}).click();
 ...
 await nav('处理配置');assert.equal(await page.locator('[data-config]:visible').count(),40);
 await page.locator('[data-config="crop_enabled"]:visible').getByRole('switch').click();
 ...
 await nav('学生与审核');
 await page.getByLabel('选择学生 00001',{exact:true}).check();await page.getByLabel('选择学生 00002',{exact:true}).check();
 ...
 await page.getByRole('checkbox',{name:'全选筛选结果',exact:true}).check();await click('生成选中照片交付包');
...
`,path.join(output,'delivery.zip')],{encoding:'utf8'});assert.equal(verify.status,0,verify.stderr);evidence.delivery=JSON.parse(verify.stdout);
 ...
 await page.getByRole('checkbox',{name:'确认这些照片已经实际发送'}).check();await click('登记已交付名单');
