...
function duration(sec){if(sec==null)return '未记录';sec=Math.max(0,Math.floor(sec));return `${Math.floor(sec/3600)}时${Math.floor(sec%3600/60)}分${sec%60}秒`;}
function time(v){return v?new Date(v).toLocaleString('zh-CN',{hour12:false}):'未记录';}
function clock(job){const t=job.timing;if(!t)return {};const n=t.ended_at||new Date(tick.value).toISOString();return {total:t.started_at&&(!['completed','cancelled'].includes(job.status)||t.ended_at)?Math.max(0,(Date.parse(n)-Date.parse(t.started_at))/1000):null,active:t.run_seconds+(t.running?Math.max(0,(tick.value-Date.parse(t.observed_at))/1000):0)};}
...
async function action(job,value){const cid=state.cohort;if(value==='cancel')await ElMessageBox.confirm('等待正在处理的项目完成并保存结果后结束任务，其余项目不再执行，本任务结束后不可继续。','安全结束',{type:'warning',confirmButtonText:'安全结束',cancelButtonText:'取消',confirmButtonClass:'el-button--danger'});busy.value=job.id;try{await api('jobs/'+job.id+'/'+value,{},undefined,cid);await refresh();ElMessage.success('操作已提交');}finally{busy.value='';}}
...
<template>
 <p class="muted">当前 {{state.cohortName}} 的任务。暂停立即停止调度，等待正在处理的项目完成后生效；安全结束是不可继续的终态。页面刷新或切换不会停止任务。</p>
 ...
 <el-card v-for="job in state.jobs" :key="job.id" shadow="never" data-testid="job" :data-job="job.id">
  <div class="section-heading"><h3>{{job.kind==='import'?'原图接收':job.kind==='delivery'?'交付打包':'照片处理'}} · {{job.id.slice(0,8)}}</h3><el-tag>{{labels[job.status]||job.status}}{{job.status==='completed'&&job.counts?.failed?'，失败 '+job.counts.failed+' 人':''}}</el-tag></div>
  <div class="job-times muted" data-testid="job-times"><span>开始时间：{{time(job.timing?.started_at)}}</span><span>结束时间：{{job.timing?.ended_at?time(job.timing.ended_at):['completed','cancelled'].includes(job.status)?'未记录':'—'}}</span><span>总历时：{{duration(clock(job).total)}}</span><span>实际运行时长：{{duration(clock(job).active)}}</span></div>
  <p v-if="job.timing?.lower_bound" class="muted">实际运行时长为已确认下限：异常中断最后一个检查点之后的时间无法准确还原，未计入停机时间。</p>
  <p v-if="job.timing?.partial" class="muted">历史计时未记录，仅累计本次恢复后的运行时长，不代表完整历史。</p>
  <p>成功 {{job.counts?.success||0}} · 失败 {{job.counts?.failed||0}} · 未执行 {{(job.counts?.remaining||0)+(job.counts?.skipped||0)}} · 总计 {{job.counts?.total||0}} · 当前 {{job.in_flight?.join('、')||job.current||'—'}}</p>
  ...
  <div class="toolbar"><el-button v-if="job.status==='running'" :loading="busy===job.id" @click="action(job,'pause')">暂停</el-button><el-button v-if="job.kind!=='delivery'&&['paused','interrupted'].includes(job.status)" :disabled="Object.keys(job.uncertain||{}).length>0" :loading="busy===job.id" @click="action(job,'resume')">继续未完成项</el-button><el-button v-if="['running','pausing','paused','interrupted'].includes(job.status)" type="danger" :loading="busy===job.id" @click="action(job,'cancel')">安全结束</el-button></div>
...
<style scoped>
.job-times{display:flex;flex-wrap:wrap;gap:8px 24px;font-size:13px;margin:8px 0 16px;}
</style>
