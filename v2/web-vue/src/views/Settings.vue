...
const dirty=computed(()=>!!baseline.value&&(serialized.value!==baseline.value||!!input.value||!!editing.value));
useDraft('settings',dirty,load);
async function load(){input.value='';editing.value=null;const d=await api('settings');records.value=d.cohorts;revision.value=d.revision;automatic.value=d.retention_days!==null;days.value=d.retention_days||30;audit.value=d.audit;deleted.value=[];baseline.value=serialized.value;trash.value=await api('recycle-bin');}
...
const blockNames={students:'学生名单',jobs:'任务记录',deliveries:'交付记录',trash:'回收站'};
function blockers(c){return Object.entries(c?.blockers||{}).map(([k,v])=>(blockNames[k]||k)+' '+v+' 条').join('、');}
async function remove(c){
 if(!c.id){records.value=records.value.filter(x=>x!==c);return;}
 const latest=await api('settings'),saved=latest.cohorts.find(x=>x.id===c.id);
 if(blockers(saved)){await ElMessageBox.alert(saved.name+' 不能删除：'+blockers(saved)+'。请在学生与审核、任务进度、照片交付或回收站处理；整届清空可在本页操作。','届次有数据',{type:'warning'});return;}
 if(dirty.value){ElMessage.warning('请先保存或放弃本页其他编辑，再删除已保存届次');return;}
 await ElMessageBox.confirm('确认永久删除空届次 '+c.name+'？','删除空届次',{type:'warning'});
 busy.value=true;try{await api('cohorts/'+c.id,{confirmed:true},'DELETE');await loadCohorts();await refresh();await load();ElMessage.success('届次已在服务器删除');}finally{busy.value=false;}
}
...
async function view(c){if(!await confirmLeave())return;await changeCohort(c.id);ElMessage.success('已选中 '+c.name+(c.archived?'（只读）':''));}
onMounted(load);
</script>
<template>
 <el-card shadow="never" :inert="busy ? '' : null"><template #header><h3>届次管理</h3><p class="muted">点击标签年份编辑；Enter 确认，Esc 取消。新增和编辑统一保存；× 删除已保存空届次时立即提交，成功后移除标签。</p></template>
