...
async function switchCohort(value){await changeCohort(value);ElMessage.success('已切换到 '+state.cohortName);}
...
async function startWorkspace(){await initialize();needsLogin.value=false;failure.value='';if(!state.cohort)page.value='settings';clearInterval(timer);timer=setInterval(()=>{if(state.ready)run(refresh);},2500);}
async function login(){const r=await fetch('/session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:loginToken.value})});if(!r.ok){failure.value='令牌错误';return;}loginToken.value='';await startWorkspace();}
function expired(){state.ready=false;needsLogin.value=true;failure.value='会话已失效，请重新登录';clearInterval(timer);}
async function logout(){await fetch('/logout',{method:'POST'});expired();failure.value='';}
onMounted(async()=>{window.addEventListener('spf-session-expired',expired);try{await startWorkspace();}catch(e){expired();failure.value='';}});
onUnmounted(()=>{clearInterval(timer);window.removeEventListener('spf-session-expired',expired);});
</script>
<template>
 <div class="app">
