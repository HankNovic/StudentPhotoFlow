...
const detail=ref(null),preview=ref(null),busy=ref(false),settings=ref([]),config=ref({}),jobs=ref([]),batches=ref([]),cohort=ref(''),cohortName=ref(''),plan=ref(null);
let timer,requestId='';
...
async function replan(){plan.value=await call('processing-jobs',{student_ids:[props.id],config:config.value,dry_run:true});requestId=crypto.randomUUID();}
async function start(){busy.value=true;try{const j=await call('processing-jobs',{student_ids:[props.id],config:plan.value.config,dry_run:false,expected_revision:plan.value.revision,request_id:requestId});plan.value=null;initialConfig.value=JSON.stringify(config.value);await load();await refresh();ElMessage.success(j.status==='skipped'?'已有结果，无需重复提交':'正式单人任务已创建，关闭窗口后继续执行');}finally{busy.value=false;}}
