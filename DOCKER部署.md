# StudentPhotoFlow 2.4.0-rc.8 · Linux Docker 部署
...
更新前在任务页点击“暂停”，等待“已暂停”（保留继续资格），或等待自然完成。“安全结束”表示永久结束该任务，不可再继续。停止容器后备份整个原挂载目录。更新 .env 中 SPF_IMAGE=ghcr.io/hanknovic/studentphotoflow:2.4.0-rc.8，其余数据路径和口令保持原值。
...
tar -czf "data-before-rc6-$(date +%Y%m%d-%H%M%S).tar.gz" data-rc4
...
首次 rc.4→rc.6 需手动刷新已打开的旧页面：rc.4 没有构建标识轮询代码，无法事后给已加载的页面添加提醒。加载 rc.6 后，后续部署会提示“服务已更新，请刷新页面”；未保存输入先确认，刷新不终止后台任务。GitHub 有新版与当前网页过期是独立判断。
...
rc.3 及更早数据仍不兼容：保留旧目录与旧镜像，在新目录部署，不自动覆盖原数据。此限制**不适用于 rc.4→rc.6**。
...
SPF_IMAGE 固定为 ghcr.io/hanknovic/studentphotoflow:2.4.0-rc.8；SPF_PORT=8769 为宿主机端口，容器内部始终 8769；SPF_DATA_DIR=./data-rc4；SPF_UPDATE_CHANNEL=candidate。如需固定 digest，将 SPF_IMAGE 改成发布说明中的 `ghcr.io/hanknovic/studentphotoflow@sha256:…`。
...
回退时停止新实例并保留新目录，使用旧镜像及与它匹配的备份。解压到独立恢复目录，修改 SPF_DATA_DIR 后启动。**换旧镜像不是数据回退**：rc.3 不支持 schema 3，rc.6 也不支持 rc.3 的旧目录；回退 rc.4 必须使用升级前整目录备份，因为 rc.4 的任务终态规则不同，不能将其恢复逻辑用于 rc.6 新记录。跨不兼容版本必须保留两套目录，不能自动迁移。
