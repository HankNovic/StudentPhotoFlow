...
SPF_PROJECT='原Compose项目名'
SPF_BACKUP_SOURCE='原数据挂载目录'
...
docker compose --env-file .env -p "$SPF_PROJECT" stop
mkdir ./data-restored-rc3
tar -xzf "$SPF_BACKUP_DIR/data.tar.gz" -C ./data-restored-rc3
# 编辑.env：SPF_DATA_DIR=./data-restored-rc3；SPF_IMAGE改为旧2.4.1-rc.3版本或记录的digest
# 管理员口令保持原值
nano .env
docker compose --env-file .env -p "$SPF_PROJECT" pull
docker compose --env-file .env -p "$SPF_PROJECT" up -d
...
回退应使用升级前的完整备份，不能假设旧镜像支持新导出格式。换回旧镜像不等于回退数据；恢复会回到备份时间点，升级后新增业务需保留并另行核对。不得使用down -v或删除旧挂载目录。备份含业务数据及口令，应私密保存。
