# StudentPhotoFlow 2.4.1-rc.2 · Linux Docker 候选部署

候选版以正式 v2.4.0 为业务基线，使用 Vue 3 + Element Plus、Notion 主题和单进程 Python。部署端无需编译，不需要 Windows EXE。默认固定镜像 ghcr.io/hanknovic/studentphotoflow:2.4.1-rc.2；本次不更新 stable/latest。固定版本不覆盖，实际 digest 见 Release。
...
3. .env 中仅将 SPF_IMAGE 改为 ghcr.io/hanknovic/studentphotoflow:2.4.1-rc.2；SPF_UPDATE_CHANNEL 可改 rc（现有检查更新入口仍只查询正式 Release）。路径、口令不变。
...
docker image inspect ghcr.io/hanknovic/studentphotoflow:2.4.0 --format '{{json .RepoDigests}}'
docker compose --env-file .env -p "$SPF_PROJECT" stop
umask 077
SPF_BACKUP_DIR="backup-before-2.4.1-rc.2-$(date +%Y%m%d-%H%M%S)"
mkdir "$SPF_BACKUP_DIR"
cp .env "$SPF_BACKUP_DIR/environment.env"
tar -czf "$SPF_BACKUP_DIR/data.tar.gz" -C "$SPF_BACKUP_SOURCE" .
# 编辑原 .env，保留路径和口令，只改镜像为2.4.1-rc.2
nano .env
...
2.4.0→2.4.1-rc.2 为兼容的增量字段升级，不重置数据；每个届次首次打开前保存 backups/before-task-timing-1.json 索引备份，完整照片仍需按上述命令备份整个目录。原子写入失败会报错，不清空或跳过原工作区。重建后需重新登录；已保存结果、配置、回收站保留期与任务进度保留。中断在途项目需人工核对后明确重试或跳过，不自动重复调用外部服务。前端提示服务已更新时先保存输入，再刷新。rc.3及更早仍不支持直接挂载，必须保留原目录另行处理；此限制不适用于2.4.0。
...
docker compose --env-file .env -p "$SPF_PROJECT" stop
mkdir ./data-restored-240
tar -xzf "$SPF_BACKUP_DIR/data.tar.gz" -C ./data-restored-240
# 编辑.env：SPF_DATA_DIR=./data-restored-240；SPF_IMAGE改为旧2.4.0版本或记录的digest
# 管理员口令保持原值
nano .env
docker compose --env-file .env -p "$SPF_PROJECT" pull
docker compose --env-file .env -p "$SPF_PROJECT" up -d
...
网页只查询GitHub正式Release并展示发布页，不拉镜像、不挂Docker socket、不停止服务。日常更新由运维主动执行pull/up；固定2.4.1-rc.2标签以后升级需改目标版本。镜像公开可匿名pull；私有包使用read:packages凭据docker login，不将凭据放源码或Compose。包管理：https://github.com/users/HankNovic/packages/container/package/studentphotoflow 。日志滚动3×10MB，健康检查/重启策略见compose.yaml。
